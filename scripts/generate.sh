#!/bin/sh
# Regenerate src/Cases.mw for ariadne.
#
#   scripts/generate.sh
#
# The crate version is pinned in scripts/generate/Cargo.toml, with the yansi
# and unicode-width versions it draws with. To move to a new one, change the
# pins and run this: if any has changed, the generator stops and says so, and
# src/ has to be brought into line with it before the fingerprint in
